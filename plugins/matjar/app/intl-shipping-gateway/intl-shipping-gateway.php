...
/**
 * Add admin button: Send payment link
 */
/*
add_action('woocommerce_admin_order_data_after_order_details', function ($order) {

    if ($order->get_payment_method() !== 'intl_shipping') return;

    if ($order->get_status() !== 'on-hold') return;

    $url = wp_nonce_url(
        admin_url('admin-post.php?action=send_payment_link&order_id=' . $order->get_id()),
        'send_payment_link'
    );

    echo '<p><a class="button button-primary" href="' . esc_url($url) . '">Send Payment Link</a></p>';
});
*/
 ...
 */
/*
add_action('admin_post_send_payment_link', function () {

    if (!current_user_can('manage_woocommerce')) return;

    $order_id = intval($_GET['order_id']);

    check_admin_referer('send_payment_link');

    $order = wc_get_order($order_id);

    // Change status to pending (ready for payment)
    $order->update_status('pending', 'Payment link sent to customer');
